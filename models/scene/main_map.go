...
func NewMap(m interfaces.GameManager) *Map {
	...
	l = layer.NewLayerBase("Layer1", ebitest.NewPoint(0, 0), fs, &color.RGBA{0, 0, 0, 128}, true)
	f.AddLayer(l)

	// スクロールビュー
	scrollView := control.NewUIScrollView("scrollView1", ebitest.NewPoint(5.0, 10.0), ebitest.NewSize(f.Size().W()-10, f.Size().H()/2))
